...
void pbkdf2_hmac_sha256(const byte* password, size_t passwordLen,
    ...
    int blocks = (keyLength + HMAC_DIGEST_SIZE - 1) / HMAC_DIGEST_SIZE;
    ...
    for (int block = 1; block <= blocks; block++) {
        ...
        size_t offset = (block - 1) * HMAC_DIGEST_SIZE;
        size_t bytesToCopy = HMAC_DIGEST_SIZE;
        if (offset + bytesToCopy > static_cast<size_t>(keyLength)) {
            bytesToCopy = keyLength - offset;
        }
        memcpy(output + offset, digest, bytesToCopy);
        
        for (int iter = 1; iter < iterations; iter++) {
            hmac_sha256(password, passwordLen, digest, HMAC_DIGEST_SIZE, digest);
