CXX = g++
CXXFLAGS = -std=c++20 -O2 -Wall -D_WIN32_WINNT=0x0A00 -DCPPHTTPLIB_NO_EXCEPTIONS -I include -I web -I third_party/httplib -I/mingw64/include
LDFLAGS = -static-libgcc -static-libstdc++ -L/mingw64/lib -l:libgmp.a -l:libwinpthread.a -lpthread -lws2_32 -lwsock32 -lcrypt32
BUILD_DIR = build
TARGET = aes_server
EMBEDDER = embed_templates.exe

.PHONY: all clean dirs generate

all: generate dirs $(TARGET)

# Шаг 1: компилируем утилиту встраивания шаблонов
$(EMBEDDER): embed_templates.cpp
	$(CXX) -std=c++20 embed_templates.cpp -o $(EMBEDDER)

# Шаг 2: запускаем утилиту — генерирует web/templates_generated.hpp
generate: $(EMBEDDER)
	./$(EMBEDDER)
...
$(BUILD_DIR)/web/templates.o: web/templates.cpp web/templates.hpp web/templates_generated.hpp web/lang.hpp
...
clean:
	rm -rf $(BUILD_DIR) $(TARGET) $(EMBEDDER) web/templates_generated.hpp